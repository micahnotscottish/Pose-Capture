...
def update_and_draw_meteors(screen, player_rect):
    """
    ...
    for meteor in meteors[:]:
        ...
        if player_rect:
            if rect_circle_collision(player_rect, meteor["x"], meteor["y"], meteor["size"]):
                if meteor["type"] == "good":
                    score += 1
                    meteors.remove(meteor)
                else:
                    # Bad meteor hit → show points and quit
                    txt = score_font.render(f"Final Score: {score}", True, (255, 255, 255))
    ...
    # Draw current score
    score_txt = score_font.render(f"Score: {score}", True, (0, 0, 0), (255,0,0))
